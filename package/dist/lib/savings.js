...
export function resolveBaselineModel() {
    const override = process.env["ZEROGPU_SAVINGS_MODEL"];
    if (override && CLAUDE_PRICING[override])
        return override;
...
export function computeCallSavings(inputTokens, outputTokens, baseline, zgpuModel) {
    const claude = pricingFor(baseline);
    const zgpu = ZGPU_PRICING[zgpuModel] ?? ZGPU_FALLBACK;
    const claudeCost = (inputTokens * claude.in) / 1_000_000 +
        (outputTokens * claude.out) / 1_000_000;
    const zgpuCost = (inputTokens * zgpu.in) / 1_000_000 + (outputTokens * zgpu.out) / 1_000_000;
    return {
        tokens: inputTokens + outputTokens,
        savingsUsd: Math.max(0, claudeCost - zgpuCost),
    };
}
export function readSavings() {
...
export function writeSavings(state) {
...
function highestMilestoneCrossed(usd) {
    let crossed = 0;
...
export function shouldShowNotice(state, random = Math.random) {
    const gap = state.totalRequests - state.notice.lastShownAtRequest;
    const milestoneCrossed = highestMilestoneCrossed(state.totalSavingsUsd) > state.notice.lastMilestoneUsd;
    if (milestoneCrossed && gap >= 2)
        return true;
    if (gap < MIN_GAP)
        return false;
    if (gap >= MAX_GAP)
...
export function recordAndMaybeNotify(input) {
    try {
        const inputTokens = input.usage?.input_tokens ?? estimateTokens(input.inputText);
        const outputTokens = input.usage?.output_tokens ?? estimateTokens(input.outputText);
        const baseline = resolveBaselineModel();
        const { tokens, savingsUsd } = computeCallSavings(inputTokens, outputTokens, baseline, input.model);
        ...
        if (show) {
            ...
            state.notice.lastMilestoneUsd = Math.max(state.notice.lastMilestoneUsd, highestMilestoneCrossed(state.totalSavingsUsd));
        }
        writeSavings(state);
        if (show)
            process.stderr.write(formatNotice(state) + "\n");
    }
    catch {
...
export function formatNotice(state) {
    return (`💰 ZeroGPU savings so far: ${fmtUsd(state.totalSavingsUsd)}  ` +
...
export function formatReport(state) {
    ...
    if (state.totalRequests === 0) {
        ...
        lines.push("");
        lines.push("Run a ZeroGPU task (chat, classify, extract, redact, summarize…) and your");
    ...
    lines.push(`Tokens offloaded: ≈ ${state.totalTokens.toLocaleString("en-US")} Claude tokens`);
    ...
    const models = Object.entries(state.byModel).sort((a, b) => b[1].savingsUsd - a[1].savingsUsd);
    if (models.length > 0) {
        lines.push("");
        lines.push("By model:");
        for (const [name, m] of models) {
            lines.push(`  ${name}  —  ${m.requests} call${m.requests === 1 ? "" : "s"}, ` +
                `${fmtUsd(m.savingsUsd)}, ${fmtTokensShort(m.tokens)} tok`);
        }
    }
    lines.push("");
    lines.push("Token counts and ZeroGPU costs are actual. Dollar savings estimate the Claude");
    lines.push(`spend avoided: what these exact tokens would have cost on ${baseline}, minus the`);
    lines.push("real ZeroGPU cost. Set ZEROGPU_SAVINGS_MODEL to compare against another Claude model.");
    return lines.join("\n");
}
