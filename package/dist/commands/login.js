...
export function registerLoginCommand(program) {
    program
        ...
        .option("--project-id <id>", "Provide your ZeroGPU project ID directly (skips the prompt).")
        ...
        if (!rawKey || !rawProjectId) {
            ...
            if (opened) {
                console.log(`Opening ${DASHBOARD_URL} in your browser — grab your Project ID and API Key from there.`);
            }
            else {
                console.log(`Tip: open ${DASHBOARD_URL} in your browser to grab your Project ID and API Key.`);
            }
        }
        if (!rawKey) {
            try {
        ...
        if (!projectResult.ok) {
            console.error(`That doesn't look like a valid project ID — ${projectResult.reason}`);
            console.error("It should be a UUID like 4ed3e5bb-c2ed-4d4a-8a66-2b161a27fd1a. Please try again.");
            process.exit(1);
        }
        ...
        if (env.shell === "windows") {
            ...
            console.log("Tip: open a new terminal window so other programs can see ZEROGPU_API_KEY.");
        }
        else {
            console.log(`We also added ZEROGPU_API_KEY to your shell config (${env.path}) so other tools can use it.`);
            console.log(`To use it right away in this terminal, run:  source ${env.path}`);
