...
function collectLabel(value, previous) {
    return previous.concat([value]);
}
export function registerExtractEntitiesCommand(program) {
    program
        .command("extract_entities <text>")
        .alias("extract-entities")
        .description("Extract named entities from text using the GLiNER2 model with custom labels.")
        .option("-l, --label <label>", "Entity label to extract (repeatable).", collectLabel, [])
        .option("--labels <labels>", "Comma-separated list of entity labels (alternative to repeating --label).")
        .option("-t, --threshold <number>", "Confidence threshold between 0 and 1.", `${DEFAULT_THRESHOLD}`)
        ...
        if (labels.length === 0) {
            console.error("At least one label is required. Use --label <name> (repeatable) or --labels a,b,c.");
            process.exit(1);
        }
...
//# sourceMappingURL=extractEntities.js.map